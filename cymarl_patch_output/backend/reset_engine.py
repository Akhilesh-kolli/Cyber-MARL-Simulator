...
def reset_entire_simulation(st, rerun=True):
    ...
    keys_to_clear = [
        ...
        "reward",
        "threat_level",
        "defender_action_history",   # FIX #6: reset RECOVER cooldown tracker
        "threat",
        "topology_instance_id",
        "overview_topology",
        "overview_soc_trend",
        "_soc_incident_report_pdf_bytes",
        "_soc_incident_report_generated",
    ]
    ...
    # Clear any generated PDF bytes so UI controls reinitialize
    try:
        if "_soc_incident_report_pdf_bytes" in st.session_state:
            del st.session_state["_soc_incident_report_pdf_bytes"]
    except Exception:
        st.session_state["_soc_incident_report_pdf_bytes"] = None
    try:
        if "_soc_incident_report_generated" in st.session_state:
            del st.session_state["_soc_incident_report_generated"]
    except Exception:
        st.session_state["_soc_incident_report_generated"] = None
