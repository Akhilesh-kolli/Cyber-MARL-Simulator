            ...
            if not ioc_df.empty and "IOC" in ioc_df.columns:
                ...
                if not tech_rows.empty:
                    ...
                    for tech, cnt in technique_counts.items():
                        ...
                        # Ports — check Ports column AND correlated "Technique on Port" IOC rows
                        ports = set()
                            ...
                            for pcell in subset["Ports"].astype(str).unique():
                                if pcell and pcell not in ("", "nan"):
                                    for p in str(pcell).split(","):
                                        ...
                                        if p and p != "nan":
                                            ports.add(p)
                        # Also extract port numbers from "T#### on Port NNNN" IOC names
                        try:
                            import re as _re
                            port_ioc_rows = ioc_df[
                                ioc_df["IOC"].astype(str).str.match(
                                    rf"^{tech}\s+on\s+Port\s+\d+", flags=_re.IGNORECASE, na=False
                                )
                            ]
                            for ioc_name in port_ioc_rows["IOC"].astype(str).unique():
                                m = _re.search(r"Port\s+(\d+)", ioc_name, _re.IGNORECASE)
                                if m:
                                    ports.add(m.group(1))
                        except Exception:
                            pass
                        ...
                        # FIX: Look up CVE for this technique from VULNERABILITY_DB
                        cve_for_tech = ""
                        cvss_for_tech = ""
                        try:
                            from utils.constants import VULNERABILITY_DB
                            for _asset, _vinfo in VULNERABILITY_DB.items():
                                if str(_vinfo.get("mitre", "")).upper() == str(tech).upper():
                                    cve_for_tech = _vinfo.get("cve", "")
                                    cvss_for_tech = str(_vinfo.get("cvss", ""))
                                    break
                        except Exception:
                            pass

                        # Also check IOC registry for CVE rows linked to this technique
                        if not cve_for_tech:
                            try:
                                cve_ioc_rows = ioc_df[
                                    (ioc_df["Type"] == "CVE") &
                                    (ioc_df.get("MITRE", pd.Series(dtype=str)).astype(str).str.upper() == str(tech).upper())
                                ]
                                if not cve_ioc_rows.empty:
                                    cve_for_tech = cve_ioc_rows.iloc[0]["IOC"]
                                    cvss_for_tech = str(cve_ioc_rows.iloc[0].get("CVSS", ""))
                            except Exception:
                                pass
                    ...
                    # FIX: Include CVE and CVSS columns in correlation table
                    _corr_col_order = ["Technique", "Count", "Severity", "CVE", "CVSS", "Assets Impacted", "Ports Observed"]
                    _raw_df = pd.DataFrame(rows)
                    _existing_corr_cols = [c for c in _corr_col_order if c in _raw_df.columns]
                    correlation_df = _raw_df[_existing_corr_cols].sort_values("Count", ascending=False).reset_index(drop=True)
